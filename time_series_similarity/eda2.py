...
# data2 = data.sample(n=15000)
# print(data2.shape)
# print(data2['s_is_fork_road'].value_counts())
# print(data2['s_is_signal_light'].value_counts())
#
...
def defineLabel(df):
    ...
    for i in range(len(signal)):
        if signal[i] == 1 and fork[i] == 1:
            res.append(0)
        elif signal[i] == 0 and fork[i] == 0 and res.count(1) < 10000:
            res.append(1)
        else:
            res.append(2)
    return res
